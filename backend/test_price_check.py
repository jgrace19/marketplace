...
class PriceCheckTests(TestCase):
    ...
    def test_price_check_rejects_missing_scheme(self) -> None:
        with self.assertRaises(HTTPException) as ctx:
            price_check(url="www.comparableprice.com")

        self.assertEqual(ctx.exception.status_code, 400)
        self.assertIn("http(s)", ctx.exception.detail)
    ...
    def test_price_check_maps_missing_schema_to_502_if_reached(self) -> None:
        with patch(
            "main.requests.get",
            side_effect=requests.exceptions.MissingSchema("no scheme"),
        ):
            with self.assertRaises(HTTPException) as ctx:
                price_check(url="https://example.com/product")
